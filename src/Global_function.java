...
public class Global_function {
               ...
               try{
                   ...
                   if(is_dokumentasi) {
                	   if(Parser_TASK.contains("LOGIN")){
                           InsTransReport(Parser_TASK,Parser_ID,Parser_SOURCE,sqlcon.EncodeString(Parser_COMMAND),Parser_OTP,Parser_TANGGAL_JAM,Parser_VERSI,sqlcon.EncodeString(Parser_HASIL),Parser_FROM,Parser_TO,Parser_SN_HDD,Parser_IP_ADDRESS,Parser_STATION,Parser_CABANG,Parser_NAMA_FILE,Parser_CHAT_MESSAGE,Parser_REMOTE_PATH,Parser_LOCAL_PATH,Parser_SUB_ID,Boolean.parseBoolean(en.getTampilkan_query_console()),"INSERT","transreport");
                       }else{
                           InsTransReport(Parser_TASK,Parser_ID,Parser_SOURCE,Parser_COMMAND,Parser_OTP,Parser_TANGGAL_JAM,Parser_VERSI,Parser_HASIL,Parser_FROM,Parser_TO,Parser_SN_HDD,Parser_IP_ADDRESS,Parser_STATION,Parser_CABANG,Parser_NAMA_FILE,Parser_CHAT_MESSAGE,Parser_REMOTE_PATH,Parser_LOCAL_PATH,Parser_SUB_ID,Boolean.parseBoolean(en.getTampilkan_query_console()),"INSERT","transreport");
                       }
                   }else {
   ...
   public void publish_main_setting() {
      	try {
           ...
           PublishMessageDocumenter(false,1,topic_dest, bytemessage, 1, res_message,1);
           PublishMessageDocumenter(false,2,topic_dest, bytemessage, 1, res_message,1);    
      	}catch(Exception exc) {
   ...
   String Parser_TASK,
    ...
    public boolean createCollection(String table,String nama_database) {
    	...
    	try {
             ...
             try {
                 //p.load(new FileInputStream("setting.ini"));
                 ip_mongodb = en.getIp_mongodb();//p.getProperty("ip_mongodb");
                 port_mongodb = en.getPort_mongodb();//p.getProperty("port_mongodb");
             } catch (Exception ex) {
   ...
   public boolean create_table_mysql(String nama_table) {
	   	boolean res = false;
	   	try {
	   		String get_syntax_generate_table = GetTransReport("SELECT CONTENT FROM setting WHERE ID = '1'", 1, true,false);
   ...
   /*
   public String InsLog(JSONObject  IN_KETERANGAN,boolean command_output) {
	   
	   String res = "";
	   try {
		 	
		   String query = "INSERT INTO transaksi_login VALUES('NULL','"+IN_KETERANGAN.toString()+"');";
		   if(command_output) {
			   System.err.println("query_log : "+query);
		   }
		   inter_login.call_upd_fetch(query,false);
		   
//		   String query = INS_OR_REPLACE+" INTO "+NAMA_TABLE+" VALUES('"+IN_CABANG+"',"
//                   + "'"+IN_TASK.toUpperCase()+"',"
//                   + "'"+IN_ID+"',"
//                   + "'"+IN_SUB_ID+"',"
//                   + "'"+IN_SOURCE+"',"
   ...
   */
   ...
   public String InsTransReport(String IN_TASK,
        ...
        try {
            ...
            }else if(NAMA_TABLE == "transreport") {
            	 query = INS_OR_REPLACE+" INTO "+nama_table_create+" VALUES('"+IN_CABANG+"',"
                         + "'"+IN_TASK.toUpperCase()+"',"
                         + "'"+IN_ID+"',"
                         + "'"+IN_SUB_ID+"',"
            ...
            try {
            	Object obj_remote = JSONValue.parse(IN_REMOTE_PATH);
            	
	            String query_log = "INSERT INTO transaksi_login VALUES(NULL,'"+obj_remote.toString()+"');";
	 		    if(command_output) {
	 			   System.err.println("query_log : "+query_log);
	 		    }
	 		    if(IN_SOURCE.equals("IDMCommander")) {
	 		    	inter_login.call_upd_fetch(query_log,false);
	 		    	res = "SUKSES INSERT LOG";
	 		    }
            }catch(Exception exc) {
     ...
     public String get_tanggal_curdate_curtime_format(){
        String res = "";
        try {
            String get = GetTransReport("SELECT NOW() AS TANGGAL",1,true,false);
            res = get;
        } catch (Exception e) {
              res = "";  
        }
      ...
      public void PublishNotCompressAndDocumenter(String topic,String pesan){
          try {
            ...
            MqttClient send_transreport = null;
                ...
                InsTransReport(Parser_TASK,Parser_ID,Parser_SOURCE,Parser_COMMAND,Parser_OTP,Parser_TANGGAL_JAM,Parser_VERSI,Parser_HASIL,Parser_TO,Parser_FROM,Parser_SN_HDD,Parser_IP_ADDRESS,Parser_STATION,Parser_CABANG,Parser_NAMA_FILE,Parser_CHAT_MESSAGE,Parser_REMOTE_PATH,Parser_LOCAL_PATH,Parser_SUB_ID,Boolean.parseBoolean(en.getTampilkan_query_console()),"INSERT","transreport");
   ...
   public boolean ChangeData(String query){
    ...
    public String GetTransReport(String query,int column,boolean return_one_columns,boolean return_json){
        String res = null;
        try{
            //con = sqlcon.get_connection_db();
            //inter_login = new Implement_ga(con);
        	if(return_json) {
        		  if(return_one_columns == true){
                      ...
                      String get = inter_login.call_get_procedure_json(query, column, false);
                      res = get;
                  }
        	}else {
        		  if(return_one_columns == true){
    ...
    /*  
    public void PublishMessageAndDocumenter(int profil,String topic,byte[] content,int counter,String plain_text_res_message,int qos){
        try {
            Date HariSekarang = new Date();
            //-- sesi koneksi db --//
            String res_broker_primary       = "";
            ...
            try{
                ...
                send_transreport.connect(options);
                MqttMessage message = new MqttMessage(content);
                //System.out.println("HASIL LOGIN : "+plain_text_res_message);
                message.setQos(qos);
                send_transreport.publish(topic, message);
                send_transreport.disconnect();
                UnpackJSON(plain_text_res_message);
...
/* Decryption Method */
    ...
    public void PrintMessage2(String FROM_THREAD,int counter,String msg_type,String topic,String Parser_TASK,String Parser_FROM,String Parser_TO,Date HariSekarang,Date HariSekarang_run){
        //=========================================================//
        System.out.println(FROM_THREAD+" : "+counter+" >> "+Parser_FROM+" >> "+Parser_TO);
    }
